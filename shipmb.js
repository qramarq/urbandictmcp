    ...
    const response = await new Promise((resolve, reject) => {
      execFile(command, argv, {
        ...
        if (error && (typeof error.code !== "number" || error.killed)) {
          reject(new Error(`ShipMB process failed: ${error.message}. Set SHIPMB_PYTHON to a Python environment with ${backend === "language" ? "ShipMBLang" : "the private shipmbcompiler library"} installed.`));
